...
Choose the OAuth2/OIDC flow by **client type**, never by convenience:
...
- **Machine-to-machine (no user present)** — **client credentials flow**. It
  yields an access token for the client itself, never a user identity.
- **Input-constrained device (TV, console, CLI on a headless box)** — **device
  ...
  its only niche; do not use it to dodge redirect-URI handling on platforms
  that support it.

Dead flows — reject in review, no exceptions:
...
4. Only after verification does the backend look up / provision the local user
   ...
   credentials, never the provider's.

**Red line: never trust a client-presented identity token without server-side
verification.** The client is attacker-controlled territory; any token it sends
...
- The provider's token is an **input** to your auth system, not your session.
  Do not store provider ID tokens as session state or accept them on ordinary
  API calls.
- Verification happens on **every** token the backend accepts as proof of
  identity, not just the first sign-in.

## 3. Token roles — what each is for, and never for

| Token | Purpose | Never use it to |
|---|---|---|
...
After the backend verifies identity it must maintain a session. Two poles:
...
**Recommended default: the hybrid.** Short-lived stateless access credential
...
- **Rotation**: every refresh issues a new refresh token and invalidates the
  old one. Mandatory for public clients; strongly recommended everywhere.
- **Reuse detection**: if a rotated-out (already-used) refresh token is
  ...
  family/session**, not just the one token, and require re-authentication.
...
- **Mobile apps**: refresh material goes in hardware-backed secure
  enclave/keystore-class storage, never plain preferences, files, or anything
...
- **Browsers**: session/refresh credentials belong in `httpOnly` + `Secure` +
  ...
  read, XSS can exfiltrate; `httpOnly` removes that entire class. If a SPA must
...
- **Servers**: client secrets and signing keys live in a secrets manager or
  injected environment — **never in version control**, container images, or
...
- Never unlink a user's last sign-in method without an alternative in place.
