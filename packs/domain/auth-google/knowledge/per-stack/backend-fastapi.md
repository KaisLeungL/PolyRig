...
defines the verification red line, JWKS caching rules, session strategy, and
the (iss, sub) identity model; this file adds only the Google specifics.

## Verification approach — pick one deliberately

Two sound implementations of "verify a Google ID token"; both must satisfy
...
- **Google's own auth library** (the default): its ID-token verifier knows
  ...
  dependency), never bare inside `async def`.
...
Either way, verification is server-side and unconditional (auth-core red
line); "the app already checked it" is never an input to this decision.
...
- Every other protected endpoint depends on the app's standard
  ...
  token presented to a regular endpoint is rejected exactly like any invalid
  credential** — the session dependency must not fall back to "try verifying
...
- **Issuer: Google legitimately uses more than one issuer string** (with and
  ...
  forms — never substring matching.
- **Clock skew: small and bounded.** Use the verifier's skew parameter
  (seconds, not minutes) rather than hand-rolled time arithmetic; never widen
...
same user never becomes two identities. `email` (only with
`email_verified` true) is a display attribute, never the key. Provisioning
runs inside one transaction as a **race-safe upsert** backed by the unique
constraint on (iss, sub); concurrent first sign-ins must yield one user row
...
- The expected audience (web client ID) — plus the Android client ID if any
  ...
  environment variables, per the stack's settings rules. Never hardcoded,
  never defaulted: a missing value must fail startup, because a backend that
...
- `.env.example` documents each variable with a placeholder (e.g.
  ...
  Client IDs are not secrets, but any Google client **secret** (server-side
...
- **401** for every verification failure on the sign-in endpoint — expired,
  ...
  missing/invalid session on regular endpoints. Do not leak WHICH check
...
- Responses use the project's single error envelope per the stack pack's
  conventions and never echo the presented token. Negative-path tests for
