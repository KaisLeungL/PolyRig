...
- Do not generate `scripts/` or `skills/` by default; both are opt-in, only when
...
A pack MAY opt-in to carry `skills/` (instructions — injected as project-level
symlinks by the consuming side) and `scripts/` (data — example scripts, copied
into the vault, never executed). **Neither is generated by default**; add them
only when the user explicitly asks. When authoring `skills/`, each
`skills/<name>/` must contain a `SKILL.md` whose frontmatter `name` equals the
directory name and carries a non-empty `description`; `validate-pack.mjs` checks
only this **structure** — the skill's content/safety is reviewed by the registry
plus git merge, not here. See `references/pack-authoring-contract.md`.
